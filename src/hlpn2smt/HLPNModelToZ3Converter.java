...
//		}
	...
	public void saveInductionCheckerToFile(String z3str) {
		String inductionChecker = buildNewCheckerFromTemplate("checkertemplate.tmpl", z3str, model.pnmlName+"InductionChecker");
		String outFileName = String.format("%sInductionChecker.c", model.pnmlName);
		try{
			OutputStream fos = new BufferedOutputStream(new FileOutputStream(outFileName));
			IOUtils.write(inductionChecker, fos);
			fos.close();
			System.out.println(model.pnmlName + "C InductionChecker saved to: " + new File(outFileName).getAbsolutePath());
		}catch (IOException e) {
            throw new RuntimeException(e);
        }
