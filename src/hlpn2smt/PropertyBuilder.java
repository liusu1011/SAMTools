...
public class PropertyBuilder {
	...
	public void set_andTypeOrigin() {
		...
		tok0.Tlist.add(new BasicType(0, 6,""));
	 ...
	 * adding property token with "*" any value of one of the data field
	 * @return
	 */
		...
		for(int i=0;i<=HLPNModelToZ3Converter.depth;i++) {
			...
			for(int p=0;p<properties.size();p++) {
				...
				for(int k=0;k<t.Tlist.size();k++){
					//some token's value field is not specified in the property, any value is in the property, so we do not need to fix the value of the token's field
					if(t.Tlist.get(k).Tstring.equals("*")){
						property.append("S"+i+placeName+"_tok_"+p+"_and["+k+"] = Z3_mk_true(ctx);"+nextline); 
						continue;
					}
	...
	/**
	 * for induction property, we need to find a path where the first k-1 state sat P, and the last state sat ~P
	 * thus, we need to negate property.
	 * @return
	 */
	public String buildInductionProperties() {
		StringBuilder property = new StringBuilder();
		property.append(nextline+"//properties"+nextline);
		property.append("Z3_ast property_and["+(HLPNModelToZ3Converter.depth+1)+"];"+nextline);
		...
		for(int i=0;i<=HLPNModelToZ3Converter.depth;i++) {
			...
			if(i < HLPNModelToZ3Converter.depth){
				property.append("property_and["+i+"] = Z3_mk_not(ctx, "+propertyFormula+");"+nextline);
			}else 
				property.append("property_and["+i+"] = "+propertyFormula+";"+nextline);
		}
		
		property.append("Z3_assert_cnstr(ctx, Z3_mk_and(ctx, "+(HLPNModelToZ3Converter.depth+1)+", property_and));"+nextline);
		return property.toString();
	}
